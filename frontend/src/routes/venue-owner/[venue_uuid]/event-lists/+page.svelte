...
<div class="w-full min-w-0 max-w-full lg:max-w-[60%] lg:mx-auto">
  ...
  {:else if loadError}
    <div class="text-center py-4 md:py-8">
      <p class="text-[12px] md:text-base text-red-600 mb-2 md:mb-4">
        {loadError}
      </p>
      <button
        on:click={() => loadData()}
  ...
  {:else}
    ...
    <div class="mb-2 md:mb-8">
      ...
      <h1
        class="text-[16px] md:text-4xl font-bold mb-0.5 md:mb-2 text-gray-900"
      >
        {venue.name}
      </h1>
      {#if owner?.name}
        <p class="text-[10px] md:text-lg text-gray-600 mb-0.5 md:mb-1">
          Owner: {owner.name}
        </p>
      {/if}
      {#if venue.address}
        <p class="text-[10px] md:text-lg text-gray-600 mb-2 md:mb-4">
          {venue.address}
        </p>
      {/if}
    </div>
    ...
    {#if venueEventLists.length === 0}
      <div class="bg-white rounded-xl shadow-lg p-2 md:p-12 text-center">
        ...
        <button
          on:click={() =>
            venue && goto(`/venue-form?venue_uuid=${venue.venue_uuid}`)}
        ...
        {#each venueEventLists as eventList (eventList.event_list_uuid)}
          ...
          >
            <div class="p-2 md:p-6">
              <div
                class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 md:gap-4 mb-2 md:mb-4"
              >
                <div class="flex-1 min-w-0">
                  ...
                  {#if eventList.date}
                    <p
                      class="text-[10px] md:text-sm text-gray-600 mb-1 md:mb-2"
                    >
                      <svg
                  ...
                  {#if eventList.comment}
                    <p
                      class="text-[10px] md:text-sm text-gray-500 whitespace-pre-line"
                    >
                      {eventList.comment}
                    </p>
                  {/if}
              ...
              {#if listEvents.length > 0}
                <div class="mt-2 md:mt-4 pt-2 md:pt-4 border-t border-gray-200">
                  <p
                    class="text-[10px] md:text-sm font-medium text-gray-700 mb-1 md:mb-2"
                  >
                    {listEvents.length} event{listEvents.length !== 1
                      ? 's'
                      : ''}
                  </p>
                  <div class="space-y-0.5 md:space-y-2">
                    {#each listEvents.slice(0, 3) as event}
                      <div
                        class="flex items-center justify-between text-[10px] md:text-sm"
                      >
                    ...
                    {#if listEvents.length > 3}
                      <p class="text-[9px] md:text-xs text-gray-500">
                        ... and {listEvents.length - 3} more
                      </p>
                    {/if}
                  </div>
                </div>
  ...
  {#if linkModalEventList}
    ...
    >
      <article
        class="bg-white rounded-xl shadow-xl max-w-2xl w-full p-3 md:p-6"
      >
        <div class="flex items-center justify-between mb-2 md:mb-4">
          <h3
            id="link-modal-title"
            class="text-[14px] md:text-xl font-bold text-gray-900"
          >
            Private Link
          </h3>
          <button
        ...
        <div class="flex gap-1 md:gap-2 mb-2 md:mb-4">
          <input
            ...
            class="flex-1 px-2 md:px-4 py-1.5 md:py-2 border-2 border-gray-300 rounded-lg bg-gray-50 text-gray-900 font-mono text-[10px] md:text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none"
          />
          <button
            on:click={() => {
